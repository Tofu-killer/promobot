...
describe('release bundle cli', () => {
  ...
  it('fails when the rollback wrapper is absent from an otherwise valid bundle input set', async () => {
    const releaseBundle = await loadReleaseBundleModule();
    ...
    expect(summary.ok).toBe(false);
    expect(summary.missing).toEqual(expect.arrayContaining(['ops/rollback-promobot.sh']));
    expect(summary.files).not.toContain('ops/rollback-promobot.sh');
    expect(summary.checksums).not.toHaveProperty('ops/rollback-promobot.sh');
  });
  ...
  it('does not bundle unlisted ops helpers outside the release bundle allowlist', async () => {
    const releaseBundle = await loadReleaseBundleModule();
    ...
    writeFile(repoRoot, 'ops/preflight-promobot.sh', '#!/usr/bin/env bash\n');
    writeFile(repoRoot, 'ops/rollback-promobot.sh', '#!/usr/bin/env bash\n');
    writeFile(repoRoot, 'ops/unlisted-helper.sh', '#!/usr/bin/env bash\n');
    writeFile(repoRoot, 'ops/verify-downloaded-release.sh', '#!/usr/bin/env bash\n');
    writeFile(repoRoot, 'ops/verify-release.sh', '#!/usr/bin/env bash\n');
    ...
    expect(summary.files).not.toContain('ops/unlisted-helper.sh');
    expect(summary.checksums).not.toHaveProperty('ops/unlisted-helper.sh');
    expect(fs.existsSync(path.join(outputDir, 'ops/unlisted-helper.sh'))).toBe(false);
  });
