...
import {
  getBrowserHandoffCompleteHelpText,
  parseBrowserHandoffCompleteArgs,
} from '../../src/server/cli/browserHandoffComplete';
import {
...
describe('browser handoff completion submitter', () => {
  ...
  it('imports a browser handoff completion through the system api when baseUrl is provided', async () => {
    ...
    try {
      ...
      expect(calls).toEqual([
        {
          url: 'http://local.test/api/system/browser-handoffs/import',
          method: 'POST',
          headers: {
            'content-type': 'application/json',
  ...
  it('fails when the handoff artifact has already been resolved', async () => {
    const { rootDir } = createTestDatabasePath();
    ...
    try {
        ...
        JSON.stringify(
          {
            ...
            status: 'resolved',
            resolvedAt: '2026-04-23T10:10:00.000Z',
      ...
      await expect(
        ...
        message: 'browser handoff artifact already resolved',
        statusCode: 409,
      });
    } finally {
      cleanupTestDatabasePath(rootDir);
