...
- 如果只是想在源码 checkout 里把 browser lane dispatch 直接回环到现有 importer / submitter，也可以显式打开 `PROMOBOT_BROWSER_LOCAL_AUTORUN=1`。在没有单独配置 `PROMOBOT_BROWSER_LANE_COMMAND` 时，内置 fallback 会默认让 `session_request` 自动回退到 `pnpm browser:lane:local`；对于 `publish_handoff` / `inbox_reply_handoff`，只有在当前 dispatch env 已经提供 `PROMOBOT_BROWSER_PUBLISH_STATUS` / `PROMOBOT_BROWSER_REPLY_STATUS` 时，才会同样自动回退到内置 local runner。若想改成你自己的包装命令，仍可显式配置 `PROMOBOT_BROWSER_LOCAL_RUNNER_COMMAND`（或对应 kind-specific / generic browser lane command）。这个 local runner 只是复用当前受管 session 文件和既有 submitter / handoff 完成契约，不等于仓库已经内置了真正的 Playwright 登录 / 发布 / 回复自动化。
