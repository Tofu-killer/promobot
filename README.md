...
- 浏览器 session 元数据既支持继续填写现有 `storageStatePath`，也支持直接导入 storage state JSON 到受管 `browser-sessions/managed/` 目录；同时 `pnpm browser:lane:local` 现在也能对 `request_session / relogin` 尝试本地 Playwright 自动化，把新抓到的 storage state 写回 canonical managed session 路径。
...
- 请求登录 / 重新登录现在会额外生成 `artifacts/browser-lane-requests/` 下的接管工单文件，便于外部 browser lane 消费；同一渠道账号同一动作若已有未结单工单，后续请求会直接复用现有工单而不再重复排队。保存 session 成功后，对应工单会回写为 `resolved`，并附上 session 摘要。`System Queue` 里的 Browser Lane 工单区也支持直接粘贴 `storageState` JSON 并调用 importer API 结单；`pnpm browser:lane:local` 则可在源码 checkout 里尝试本地 Playwright 登录 / 刷新。但如果本机没有安装 Playwright，或自动化结束后仍未捕获到满足内置平台 cookie heuristic 的 authenticated storage state，local runner 会直接 fail closed，不会拿旧 managed session 冒充成功。
...
- 如果只是想在源码 checkout 里把 browser lane dispatch 直接回环到现有 importer / submitter，也可以显式打开 `PROMOBOT_BROWSER_LOCAL_AUTORUN=1`。真实优先级是 kind-specific command > `PROMOBOT_BROWSER_LANE_COMMAND` > opt-in local autorun；只有在 no kind-specific / generic command 且 local autorun 已启用时，`PROMOBOT_BROWSER_LOCAL_RUNNER_COMMAND` 才会被读取。若想改成你自己的包装命令，也可以显式配置 `PROMOBOT_BROWSER_LOCAL_RUNNER_COMMAND`；但这个自定义 command 会对任意 browser lane dispatch 直接生效，不会再替你保留 publish / reply 的状态门槛，所以相关 gating 需要由你的 wrapper 自己保证。内置 local runner 对 `session_request / relogin` 会尝试本地 Playwright 自动化，并只在 submit 成功后才把抓到的 storage state 提升为 canonical managed session；如果本机没有安装 Playwright，或自动化结束后仍未捕获到满足内置 cookie heuristic 的 storage state，则会直接 fail closed。对 `publish_handoff / inbox_reply_handoff` 仍只是复用既有 handoff completion 契约，不等于仓库已经内置了真正的自动发布 / 自动回复流程。
...
- `pnpm release:local -- [options]`：先按需执行 `pnpm build`，再调用 `release:bundle` 生成目录型 release bundle；随后在同级额外产出 `tar.gz` archive、`.sha256` sidecar、`.metadata.json` metadata sidecar，以及 standalone `verify-downloaded-release.sh` helper，并在脚本结束前用这份 helper 对刚打出的下载包做一次本地自检
...
- `pnpm preflight:local -- [options]`：先跑 `preflight:prod` 的 production preflight + config validation，并把 `--require-env` 这类参数透传过去，再按需追加独立 smoke check
...
- 正式 `v*` tag push 会在保留 Actions artifact 的同时追加 `release asset`，并基于对应的 archive、`.sha256` sidecar、`.metadata.json` metadata sidecar 自动生成 `release body`，也就是 Release 页面自带的 download / verify 说明；现在这段给人读的页面说明也会顺带写出 `prerelease` 和 `test_execution.summary` 对应的测试执行状态语义，方便人工核对。
...
- 要注意，`release body` 只是给人读的页面说明，不等于页面下方真正可下载的 `release asset`；workflow 真正附着的是 archive、checksum sidecar 和 metadata sidecar。即使 `release body` 现在会显示 `prerelease` 和测试执行状态语义，机器可读发布上下文仍以 `.metadata.json` metadata sidecar 里的 `prerelease` 和 `test_execution` 等字段为准，它不替代 metadata sidecar。
- `release asset` 下载链路里的 `.metadata.json` metadata sidecar 仍是给下载方 / 自动化方消费的机器可读说明，用来描述这次 `Release Bundle` 产物及校验入口，并区分 `schema_version`、`checksum_algorithm`、`archive_format`、`run_url`、`release_url`、`test_execution.state`、`test_execution.mode`、`test_execution.summary` 这些发布上下文字段；它不替代 tar.gz 的 sidecar，也不替代 workflow run 页面里给人读的 `summary`，更不替代解压后的 `manifest.json` 校验
...
- `pnpm release:deploy` / `ops/deploy-release.sh` 面向“目标机只拿到目录型 release bundle”的部署：bundle 会随产物带上 deploy 脚本和根目录的 `start-promobot.sh`，解压后直接在 bundle 根目录执行即可。
...
pnpm verify:downloaded-release -- --archive-file /tmp/promobot-release.tar.gz

# 或直接复用同级产出的 standalone helper
bash /tmp/verify-downloaded-release.sh --archive-file /tmp/promobot-release.tar.gz
...
如果下载方只拿到 release bundle，不需要回到源码仓库重新找命令入口。bundle 根目录现在自带 `start-promobot.sh`，首次启动时会自动完成最小化 bootstrap：

- 若 bundle 根目录还没有 `.env`，会先从 `.env.example` 复制一份
- 若 `.env` 里的 `ADMIN_PASSWORD` 仍是 `change-me`，会自动生成随机密码写回 `.env`
- 若机器没有全局 `pnpm`，脚本会自动回退到 `corepack pnpm`
...
首次启动后应尽快打开 bundle 根目录的 `.env`，记录并轮换自动生成的 `ADMIN_PASSWORD`。

## 生产运维补充

- `pm2.config.js` 现在会把日志落到仓库下的 `logs/`，并带基本重启/退避配置。
- `ops/release-promobot.sh` 现在提供一条本地 release 打包脚本；它会先按需构建，再调用 `release:bundle` 生成目录型 release bundle，并在同级额外产出 `tar.gz`、`.sha256`、`.metadata.json` 和 standalone `verify-downloaded-release.sh` helper；脚本结束前还会用这份 helper 对刚打出的 archive 做一次本地自检。
- `ops/deploy-release.sh` 现在提供 bundle 内直接部署入口；进入 release bundle 根目录后优先运行 `bash ./start-promobot.sh`，也可以显式调用 `pnpm release:deploy`。
...
- `ops/preflight-promobot.sh` 现在提供上线前预检脚本；它会先跑 `preflight:prod` 的 production preflight + config validation，再按需追加独立 smoke check；这条 wrapper 不等于 readiness check。在源码仓库里会走 `pnpm preflight:prod` / `pnpm smoke:server`，在已解压的 bundle 根目录里则会自动切到 bundle 自带的 `dist/server/cli/preflightPromobot.js` / `dist/server/cli/deploymentSmoke.js`。
