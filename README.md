...
- `ADMIN_PASSWORD` 现在通过 `x-admin-password` 请求头接入后端 API；前端首次进入时会要求输入管理员密码。默认保存在浏览器 `sessionStorage`（当前标签页会话级）中；如果勾选“记住这台浏览器”，则会改为长期保存。
...
- 仓库同时提供了 `pnpm browser:lane:submit -- --request-artifact <path> --storage-state-file <path>`，用于从本地 Playwright storage state JSON 生成 `browser_lane_result` artifact；如果再附带 `--base-url` 和 `--admin-password`，会立即调用 importer API。这个 CLI 适用于和服务端共享同一份部署目录 / artifact 卷的 browser lane。
