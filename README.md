...
- 如果只是想在源码 checkout 里把 browser lane dispatch 直接回环到现有 importer / submitter，也可以显式打开 `PROMOBOT_BROWSER_LOCAL_AUTORUN=1`。在没有单独配置 `PROMOBOT_BROWSER_LANE_COMMAND` 时，内置 fallback 仍只会让 `session_request` 自动回退到 `pnpm browser:lane:local`；如果希望 publish handoff / reply handoff 也自动接入本地 runner，需要额外显式配置 `PROMOBOT_BROWSER_LOCAL_RUNNER_COMMAND`（或对应 kind-specific / generic browser lane command）指向你自己的包装命令。这个 local runner 只是复用当前受管 session 文件和既有 submitter / handoff 完成契约，不等于仓库已经内置了真正的 Playwright 登录 / 发布 / 回复自动化。它仍然只会为 `session_request` 自动补默认结果 env；publish handoff 或 reply handoff 无论是自动 dispatch 还是手工调用，都仍必须显式提供 `PROMOBOT_BROWSER_PUBLISH_STATUS` 或 `PROMOBOT_BROWSER_REPLY_STATUS`，避免误结单。
