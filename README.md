...
- 手动 preview run（`workflow_dispatch`）主要产出可下载的 Actions artifact，不额外承诺发布 `release asset`，也不会新建或更新 GitHub Release；因此它不会影响 `prerelease` 状态。
