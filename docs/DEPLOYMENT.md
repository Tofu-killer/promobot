...
- `PROMOBOT_DB_PATH`
  ...
  - 如果只是想在源码 checkout 里把现有 dispatch 默认回环到本机 submitter，可设置 `PROMOBOT_BROWSER_LOCAL_AUTORUN=1`，且不单独配置 `PROMOBOT_BROWSER_LANE_COMMAND`。此时 `session_request` 会自动回退到 `pnpm browser:lane:local`；`publish_handoff` / `inbox_reply_handoff` 只有在 dispatch env 已经提供 `PROMOBOT_BROWSER_PUBLISH_STATUS` / `PROMOBOT_BROWSER_REPLY_STATUS` 时才会同样自动回退。若想替换包装命令，可改配 `PROMOBOT_BROWSER_LOCAL_RUNNER_COMMAND`。这个 local runner 只会基于当前 managed session 文件和既有 importer / handoff 完成契约调用 `browser:lane:bridge`，不等于仓库已经内置了真正的 Playwright 自动登录、自动发布或自动回复
