...
- `PROMOBOT_DB_PATH`
  ...
  - 如果只是想在源码 checkout 里把现有 dispatch 默认回环到本机 submitter，可设置 `PROMOBOT_BROWSER_LOCAL_AUTORUN=1`，且不单独配置 `PROMOBOT_BROWSER_LANE_COMMAND`。真实优先级是 kind-specific command > `PROMOBOT_BROWSER_LANE_COMMAND` > opt-in local autorun；只有在 no kind-specific / generic command 且 local autorun 已启用时，`PROMOBOT_BROWSER_LOCAL_RUNNER_COMMAND` 才会被读取。若想替换包装命令，可改配 `PROMOBOT_BROWSER_LOCAL_RUNNER_COMMAND`；但这个自定义 command 会对任意 browser lane dispatch 直接生效，不会再替你保留 publish / reply 的状态门槛，所以相关 gating 需要由 wrapper 自己保证。这个 local runner 对 `session_request / relogin` 会尝试本地 Playwright 自动化，并且只在 submit 成功后才把抓到的 storage state 提升为 canonical managed session；如果本机没有安装 Playwright，或自动化结束后仍未捕获到满足内置 cookie heuristic 的 storage state，则会直接 fail closed。`publish_handoff / inbox_reply_handoff` 仍只是复用既有 handoff completion 契约，不等于仓库已经内置了真正的自动发布或自动回复
